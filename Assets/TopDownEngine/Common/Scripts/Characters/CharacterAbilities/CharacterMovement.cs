...
{	
	...
	public class CharacterMovement : CharacterAbility 
	{
        ...
        [MMInformation("基础移速加成 主要用于区分同种怪物 属性的偏差",MMInformationAttribute.InformationType.Info,false)]
        public float additiveMoveSpeed;
        public float minAddtiveMoveSpeed;
        public float maxAddtiveMoveSpeed;
        public float MovementSpeed
        {
            get
            {
                ...
                return characterConfigure.characterMoveSpeed+characterConfigure.additiveMoveSpeed+additiveMoveSpeed;
            }
            set
            {
                movementspeed = value;
        ...
        protected override void Initialization()
		{
          ...
          if(this.tag==Tags.Enemies|| (gameObject.tag == Tags.Patner && GetComponent<PatnerController>()._summonType == SummonType.NonSummon))
            {
                additiveMoveSpeed = Random.Range(minAddtiveMoveSpeed, maxAddtiveMoveSpeed+1);
                additiveMoveSpeed += (MovementSpeed * LevelUnitManager.Instance.ReturnCurrentLevelStrength() * 0.005f);
            }
		} 
		...
		{
            if (GameManager.Instance.PlayingTimeline)
            {
               _movement.ChangeState(CharacterStates.MovementStates.Idle);
            }

            if(GameManager.Instance.Paused||GameManager.Instance.PlayingTimeline)
            {
                 StopAbilityUsedSfx();
                 return;
            }
            ...
            {
                  EventTypeManager.Broadcast(HTEventType.AddIdleDefence);
            }
        ...
        private void OnDestroy()
        {
            if(brokenSpeed!=0)
            {
                characterConfigure.additiveMoveSpeed += brokenSpeed;
                brokenSpeed = 0;
            }
