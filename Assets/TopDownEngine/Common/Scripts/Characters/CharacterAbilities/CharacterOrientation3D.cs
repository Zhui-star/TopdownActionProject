...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterOrientation3D : CharacterAbility
    {
        ...
        {
            base.ProcessAbility();
            if (GameManager.Instance.PlayingTimeline||GameManager.Instance.Paused)
            {
                return;
            }
        ...
        public void HTForceRotation(Transform target, bool smooth = false, float rotateSpeed = 10)
        {
            ...
            if(!smooth)
            {
                _newMovementQuaternion = Quaternion.LookRotation(newTargetPosition - transform.position);
            }
            else
            {
                _tmpRotation = Quaternion.LookRotation(newTargetPosition - transform.position);
                _newMovementQuaternion = Quaternion.Slerp(MovementRotatingModel.transform.rotation, _tmpRotation
                    , Time.deltaTime * rotateSpeed);

                Debug.DrawRay(transform.position, (newTargetPosition - transform.position) * 10,Color.red);
            }

            RotateModel();

            _controller.CurrentDirection = MovementRotatingModel.transform.forward;
        }
        /// <summary>
		...
		protected virtual void RotateToFaceWeaponDirection()
        {
            ...
            _controller.CurrentDirection = _rotationDirection;
        }
