...
namespace MoreMountains.TopDownEngine
{
    ...
    public class Projectile : MMPoolableObject
    {
        ...
        [Header("蓄力效果")]
        public WeaponAccumlateResult _weaponAccumlateReuslt;
        public float _accumalateMultiple;
        private Vector3 _orginLocalScale;
		...
		protected override void OnDisable()
        {
            ...
            ResetWeaponAccumlate();
        }

        /// <summary>
        /// 处理武器蓄力
        /// </summary>
        void HandlerWeaponAccumlate()
        {
            if (_weapon != null)
            {

                if (_weapon.AccumlateWeapon)
                {
                    Accumlate = true;
                    switch (_weaponAccumlateReuslt)
                    {
                        case WeaponAccumlateResult.MultipleScale:
                            _orginLocalScale = this.transform.localScale;
                            this.transform.localScale *= _accumalateMultiple;
                            break;
                    }
                    _weapon.AccumlateWeapon = false;
                }
            }
        }
        ...
        /// 重置武器蓄力效果
        /// </summary>
        void ResetWeaponAccumlate()
        {
            switch (_weaponAccumlateReuslt)
            {
                case WeaponAccumlateResult.MultipleScale:
                    this.transform.localScale = _orginLocalScale;
                    break;
            }
            Accumlate = false;
