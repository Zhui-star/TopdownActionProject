...
namespace MoreMountains.TopDownEngine
{
    ...
    public class AIDecisionDetectTargetRadius3D : AIDecision
    {
        ...
        protected virtual bool DetectTarget()
        {
            ...
            if (_hit.Length > 0)
            {
                Transform closetTarget=GetClosetObject(_hit);
                // we cast a ray to make sure there's no obstacle
                _raycastDirection = closetTarget.position - _raycastOrigin;
                RaycastHit hit = MMDebug.Raycast3D(_raycastOrigin, _raycastDirection, Vector3.Distance(closetTarget.transform.position, _raycastOrigin), 
                    ObstacleMask.value, Color.yellow, true);
                if (hit.collider == null)
                {
                    _brain.Target = closetTarget.transform;
                    return true;
                }
                else
                {
        ...
        /// <summary>
        /// Get closet target from targets
        /// </summary>
        /// <param name="targets">All object be dected</param>
        /// <returns></returns>
        Transform GetClosetObject(Collider[] targets)
        {
            Transform closetTarget=null;
            float closetDistance=Mathf.Infinity;
            for(int i=0;i<targets.Length;i++)
            {
                float currentDistanceSqr=Vector3.SqrMagnitude(transform.position-targets[i].transform.position);
                if(currentDistanceSqr<=closetDistance*closetDistance)
                {
                    closetTarget=targets[i].transform;
                    closetDistance=Mathf.Sqrt(currentDistanceSqr);
                }
            }
