...
{	
	...
	{	
        ...
        protected virtual void InstantiatePlayableCharacters()
		{
			...
			{ 
				foreach (Character playerPrefab in PlayerPrefabs)
				{
                    ...
                    HTCharacterSkill characterSkill = newPlayer.gameObject.GetComponent<HTCharacterSkill>();
                    if (characterSkill != null)
                    {
                        LearnSkill(characterSkill);
                    }
        ...
        /// 初始化技能
        /// </summary>
        void LearnSkill(HTCharacterSkill characterSkill)
        {
            if (HTLevelManager.Instance.ReturnLearenedSkills().Count <= 0) return;
