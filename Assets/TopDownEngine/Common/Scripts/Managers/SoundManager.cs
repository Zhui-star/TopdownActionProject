...
namespace MoreMountains.TopDownEngine
{
    ...
    public class SoundManager : MMPersistentSingleton<SoundManager>, MMEventListener<TopDownEngineEvent>, MMEventListener<MMGameEvent>
    {
        ...
        public virtual void PlayBackgroundMusic(AudioSource Music, bool shouldBool = true,bool InitialPause=false)
        {
        ...
        public virtual AudioSource PlaySound(AudioClip sfx, Vector3 location, bool loop = false )
        {
            ...
            if (GameManager.Instance.Paused&& GameObject.Find("TempAudio")) // When game paused we not suggestion spawn new gameobject for new sound
            {
                temporaryAudioHost = GameObject.Find("TempAudio");
            }
            else
            {
            ...
            if (!temporaryAudioHost.GetComponent<AudioSource>())
            {
                audioSource = temporaryAudioHost.AddComponent<AudioSource>() as AudioSource;
            }
            else
            {
                audioSource = temporaryAudioHost.GetComponent<AudioSource>();
            }      
            ...
            AudioSource reuseSource = null, AudioMixerGroup audioGroup = null)
        {
            ...
            if (loop)
            {
                if(!_loopingSounds.Contains(audioSource))
                {
                    _loopingSounds.Add(audioSource);
                }
        ...
        public virtual void PauseLoopingSound(AudioSource source)
        {
            if(source!=null)
            {
               AudioSource audioSource=    _loopingSounds.Find((x) => x == source);
              audioSource.Pause();
            }
        }
        ...
        /// 0<volume<1  调整音效开关
        /// </summary>
        /// <param name="voloume"></param>
        public void AdjustmentAudio(float voloume)
        {
            if(_backgroundMusic == null) { return; }
            MusicVolume = voloume;
            _backgroundMusic.volume = MusicVolume;
            PlayerPrefs.SetFloat(Consts.Music, MusicVolume);
            PlayerPrefs.Save();
        }
        ...
        public void AdjustmentSfx(float volume)
        {
            SfxVolume = volume;
            PlayerPrefs.SetFloat(Consts.Sfx, SfxVolume);
            PlayerPrefs.Save();
        }
        #endregion
        ...
        //convinient connect for SFX and BgMusic
        public void isOnSfxOrNot(bool status)
        {
            if(status) { SfxOn(); }
            else { SfxOff(); }
        }
        ...
        public void isOnBgMusicOrNot(bool status)
        {
            if(status) { MusicOn(); }
            else { MusicOff(); }
        }
		...
		protected virtual void LoadSoundSettings()
        {
            ...
            if(PlayerPrefs.HasKey(Consts.Music))
            {
                MusicVolume = PlayerPrefs.GetFloat(Consts.Music);
            }else
            {
                MusicVolume = 0.5f;
                isOnBgMusicOrNot(true);
            }

            if(PlayerPrefs.HasKey(Consts.Sfx))
            {
                SfxVolume = PlayerPrefs.GetFloat(Consts.Sfx);
            }else
            {
                SfxVolume = 0.5f;
                isOnSfxOrNot(true);
