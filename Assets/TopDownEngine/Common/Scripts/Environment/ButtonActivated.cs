...
namespace MoreMountains.TopDownEngine
{
    ...
    public class ButtonActivated : MonoBehaviour
    {
        ...
        [HideInInspector]
        public ButtonPrompt _buttonPrompt;
        ...
        public virtual void HidePrompt()
        {
            ...
            if(_buttonPrompt.gameObject.activeInHierarchy)
            {
                _buttonPrompt.Hide();
            }
        }
        ...
        protected virtual void TriggerEnter(GameObject collider)
        {
            ...
            if (ShouldUpdateState)
            {
                ...
                {
                      if(!_characterButtonActivation.GetActivateds().Contains(this))
                      {
                        _characterButtonActivation.AddButtonAcitvateds(this);
                       Debugs.LogInformation("Can Trigger interaction count:" + _characterButtonActivation.GetActivateds().Count, Color.blue);
                    }
        ...
        /// <summary>
        /// Remove this button iteraction gameobject from buttonActivated list of character
        /// </summary>
        void RemoveThisFromButtonAcitivats()
        {
            if (_characterButtonActivation != null)
            {
                if (_characterButtonActivation.GetActivateds().Count > 0)
                {
                    if (_characterButtonActivation.GetActivateds().Contains(this))
                    {
                        _characterButtonActivation.RemoveActivated(this);
                    }
                }
            }
        }
