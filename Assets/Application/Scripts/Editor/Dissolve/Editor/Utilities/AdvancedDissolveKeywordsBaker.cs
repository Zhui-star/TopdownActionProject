            ...
            using (new AmazingAssets.EditorGUIUtility.EditorGUILayoutBeginVertical(EditorStyles.helpBox))
            {
                if (cutoutGeometricType == AdvancedDissolve.AdvancedDissolveKeywords.CutoutGeometricType.Cube && globalControlID == AdvancedDissolve.AdvancedDissolveKeywords.GlobalControlID.None)
                {
                    using (new AmazingAssets.EditorGUIUtility.EditorGUILayoutBeginHorizontal())
                    {
                        ...
                        if(errorIcon == null)
                            errorIcon = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(Utilities.GetPathToTheAssetInstallationtFolder(), "Editor", "Icons", "erroricon.png"), typeof(Texture2D));
                ...
                else
                {
                    ...
                    using (new AmazingAssets.EditorGUIUtility.GUIBackgroundColor(isReadyToBake ? Color.white : Color.red))
                    {
                        newShaderName = EditorGUILayout.TextField("Shader Name", newShaderName);
                    }

                    using (new AmazingAssets.EditorGUIUtility.GUIEnabled(isReadyToBake))
                    {
                        ...
                        using (new AmazingAssets.EditorGUIUtility.GUIBackgroundColor(buttonColor))
                        {
        ...
        static string BakeShaderForBatcher(Material material, string shaderName)
        {
            ...
            for (int i = 0; i < newShaderFile.Count; i++)
            {
                ...
                if (newShaderFile[i].Contains("shader_feature_local"))
                {
                    ...
                    {
                        newShaderFile[i] = "#define _ADVANCED_DISSOLVE_KEYWORDS_BAKED" + System.Environment.NewLine + System.Environment.NewLine + "#pragma shader_feature _AD_STATE_ENABLED";
                        i += 1;
                    }
