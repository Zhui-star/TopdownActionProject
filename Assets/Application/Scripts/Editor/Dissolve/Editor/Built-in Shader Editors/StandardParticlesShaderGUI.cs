...
namespace UnityEditor
{
    internal class AdvancedDissolve_StandardParticlesShaderGUI : ShaderGUI
    {
        ...
        public void FindProperties(MaterialProperty[] props)
        {
            ...
            //Advanced Dissolve
            AmazingAssets.AdvancedDissolveEditor.MaterialEditor.Init(props);
        }
        ...
        public void ShaderPropertiesGUI(Material material)
        {
            ...
            lightingEnabled = material.shader.name.Contains("Unlit") ? false : true;

            AmazingAssets.AdvancedDissolveEditor.MaterialEditor.DrawCurvedWorldHeader(m_MaterialEditor, material);


            if (AmazingAssets.AdvancedDissolveEditor.MaterialEditor.DrawDefaultOptionsHeader("Default Shader Options", material, null))
            {
            ...
            AmazingAssets.AdvancedDissolveEditor.MaterialEditor.DrawDissolveOptions(m_MaterialEditor, false, false, true, false, false);
        ...
        void DoVertexStreamsArea(Material material)
        {
            ...
            if (GUILayout.Button(Styles.streamApplyToAllSystemsText, EditorStyles.miniButton, GUILayout.ExpandWidth(false)))
            {
                ...
                foreach (ParticleSystemRenderer renderer in m_RenderersUsingThisMaterial)
                {
                    if (renderer != null)
                    {
                        if (useGPUInstancing && renderer.renderMode == ParticleSystemRenderMode.Mesh)
